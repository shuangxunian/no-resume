...
const url = useActiveObjectModel("url")
const newUrl = ref();
watchEffect(() => {
    newUrl.value = url.value.modelValue
})
watchEffect(() => {
    url.value.onChange(newUrl.value)
})
...
watchEffect(() => {
  ...
  opacity.value = isNumber(fill.value.modelValue.opacity)
    ? fill.value.modelValue.opacity * 100
    : 100;
  if (hasRotation) {
    rotation.value = isNumber(fill.value.modelValue.rotation)
      ? fill.value.modelValue.rotation
      : 0;
  } else {
    rotation.value = 0;
  }
  if (hasScale) {
    scale.value = isNumber(fill.value.modelValue.scale)
      ? fill.value.modelValue.scale
      : 100;
  } else {
    scale.value = 100;
  }
