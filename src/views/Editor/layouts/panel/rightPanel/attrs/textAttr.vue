...
const fontFamily = useActiveObjectModel<
  "fontFamily",
  SelectProps["modelValue"]
>("fontFamily");
const fontSize = useActiveObjectModel<"fontSize", SelectProps["modelValue"]>(
  "fontSize"
); // 字号
const textWrap = useActiveObjectModel<"textWrap", SelectProps["modelValue"]>(
  "textWrap"
); // 文本换行规则
...
const changeTextOverflow = (val) => {
  if (["show", "hide"].includes(val)) {
    ...
    if (
      ["show", "hide"].includes(textOverflowVal.value) ||
      ...
      <a-space direction="vertical" size="mini">
        <a-row :gutter="[4, 4]">
          ...
          <a-col>
            ...
            >
              <template #prefix> 文本换行规则 </template>
            </a-select>
          </a-col>
        </a-row>
        <a-row :wrap="false" :gutter="[4, 4]" align="center">
          <a-col flex="none">
            <a-tooltip mini position="bottom">
              ...
              >
                <template #icon>
                  <icon-italic
                    :class="italic.modelValue ? 'arco-icon-check' : ''"
                  />
                </template>
              </a-button>
              <template #content>
        ...
        <a-row :wrap="false" :gutter="[4, 4]" align="center">
          <a-col flex="none">
            <a-radio-group
              type="button"
              size="small"
              v-model="textOverflowType"
              @change="changeTextOverflow"
            >
              <a-radio value="show"> 超出显示 </a-radio>
              <a-radio value="hide"> 超出隐藏 </a-radio>
              <a-radio value="custom">
                ...
                <a-popover title="超出显示" trigger="click" position="lt">
                  ...
                  <template #content>
                    <a-input
                      v-model="textOverflowVal"
                      @input="inputTextOverflowVal"
                    ></a-input>
                  </template>
                </a-popover>
              </a-radio>
